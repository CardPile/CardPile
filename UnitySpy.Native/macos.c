...
unsigned char* read_process_memory(int pid, mach_vm_address_t addr, mach_msg_type_number_t* size)
{
    ...
    // Use vm_read, rather than mach_vm_read, since the latter is different in iOS.
    kern_return_t kr = vm_read(
        t,         // vm_map_read_t target_task,
    ...
    if (kr)
    {
        perror("Call to vm_read failed. Error:");
        perror(mach_error_string(kr));
        return NULL;
    }
...
{
    char* result = (char*)read_process_memory(pid, addr, &size);
    if(result == NULL)
    {
        perror("read_process_memory_to_buffer - failed to read memory");
        return 1;
    }
...
char* get_module_info(int pid, const char* module_name, int* module_size, char* path)
{
    ...
    for (int i=0; i < infos->infoArrayCount; i++)
    {
        ...
        char* fpath_addr = (char*)read_process_memory(pid, (mach_vm_address_t) info[i].imageFilePath, &size3);
        ...
        if (str_ends_with(fpath_addr, module_name))
        {
            ...
            header = (struct mach_header_64*)read_process_memory(pid, (mach_vm_address_t) info[i].imageLoadAddress, &size4);
            ...
            return (char*)info[i].imageLoadAddress;
        }
    }
    return 0;
}
...
int get_modules(int pid, module_info_t* modules, int count)
{
    ...
    for (i = 0; i < infos->infoArrayCount && i < count; i++)
    {
        ...
        mach_msg_type_number_t size4 = sizeof(struct mach_header_64);
        header = (struct mach_header_64*)read_process_memory(pid, module->address, &size4);
